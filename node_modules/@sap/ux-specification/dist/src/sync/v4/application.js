...
const getSettingsPathForFooterOP = (pageName) => {
    return exports.getSettingsPathForPage(pageName) + `/content/footer`;
};
...
const getSelectionFieldsPath = (pageName) => {
    return exports.getControlConfigurationPath(pageName) + `/@${"com.sap.vocabularies.UI.v1.SelectionFields" /* SelectionFields */}`;
};
...
const getFilterFieldPath = (pageName, _manifest, _targetAnnotationEncoded, fieldId) => {
    return exports.getSelectionFieldsPath(pageName) + `/filterFields/${fieldId}`;
};
...
const getColumnsPathForView = (pageName, _manifest, _targetAnnotationEncoded, columnKey, targetAnnotation) => {
    const annotationEncoded = targetAnnotation.replace(/\//gi, '??');
    return exports.getControlConfigurationPath(pageName) + `/${annotationEncoded}/columns/${columnKey}`;
};
...
const getActionsPathForView = (pageName, _manifest, _targetAnnotationEncoded, columnKey, targetAnnotation) => {
    const annotationEncoded = targetAnnotation.replace(/\//gi, '::');
    return exports.getControlConfigurationPath(pageName) + `/${annotationEncoded}/actions/${columnKey}`;
};
...
__decorate([
    decorators_1.syncRule(controllerExtensions_1.controllerExtensions)
