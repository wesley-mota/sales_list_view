...
function getVisualFilters(v4Page) {
    const selectionFields = v4Page.options.settings?.controlConfiguration?.[`@${"com.sap.vocabularies.UI.v1.SelectionFields" /* SelectionFields */}`]?.['filterFields'];
    if (selectionFields) {
        const visualFilters = {};
    ...
    for (const fieldName in visualFiltersManifest) {
        ...
        if (!filterField.template) {
            const visualFiltersField = (visualFilters.properties[fieldName] = {
                $ref: common_1.getFacetDefinitionLink(ux_specification_types_1.DefinitionName.VisualFilter, fieldName)
            });
            ...
            const filterDefinition = common_1.parseSchemaDefinition(ux_specification_types_1.DefinitionName.VisualFilter, fieldName, appSchema);
...
function adaptTableDefinition(appSchema, entityType, generateParameters, templateAnnotation, v4Page, alias, tableDefinitionName) {
    ...
    if (visualization) {
        ...
        const sectionIdInManifest = annotationTermQualifier
            ? `@${"com.sap.vocabularies.UI.v1.LineItem" /* LineItem */}#${annotationTermQualifier}`
            : `@${"com.sap.vocabularies.UI.v1.LineItem" /* LineItem */}`;
        ...
        tableDefinition[ux_specification_types_1.SchemaTag.annotationPath] = common_1.createAnnotationPath(entityType.fullyQualifiedName, term, lineItemAnnotation?.qualifier);
        // Table toolbar actions with custom actions
...
function addHeaderActions(appSchema, v4Page, logger) {
    ...
    utils_1.addHeaderFooterCustomActionDefinition(appSchema, v4Page, logger, customActionDefinitionKey, headerActionsDefinitionKey);
    ...
    utils_1.addEnumForActionAnchor(appSchema, headerActionsDefinitionKey, facetKey);
}
/**
...
function generateListReportSchemaV4(generateParameters, genericSchema, templateName, contextPath) {
    ...
    if (generateParameters.manifest) {
        ...
        const v4Page = utils_1.findPageV4(pages, templateName, generateParameters.entitySet, contextPath, generateParameters.logger);
        if (!v4Page) {
            return appSchema;
        }
