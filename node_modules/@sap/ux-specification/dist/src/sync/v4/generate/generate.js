 ...
 */
function generateObjectPageInterfaceV4(genericSchema, entitySetName, annotations, manifest, logger, fragments, contextPath) {
    const serviceAVT = utils_1.parseAndMergeAndConvert(annotations, logger);
    const { entitySet, entityType } = importProject_1.getEntityTypeEntitySetFromContextPath(serviceAVT, contextPath, entitySetName);
    ...
    return _1.generateObjectPageSchemaV4(generateParameters, genericSchema, contextPath);
}
...
function generateListReportInterfaceV4(templateType, genericSchema, entitySetName, annotations, logger, manifest, contextPath) {
    ...
    return _1.generateListReportSchemaV4(generateParameters, genericSchema, templateName, contextPath);
}
...
function generateFPMCustomPageInterfaceV4(genericSchema, routingId, manifest, views, annotations, featureToggles, logger) {
    ...
    if (utils_1.isFeatureSupported(featureToggles, ux_specification_types_1.Features.BuildingBlocks)) {
        const serviceAVT = utils_1.parseAndMergeAndConvert(annotations, logger);
        const viewData = utils_2.getPageCustomViewFile(views || [], routingId, manifest);
        schema = viewData?.file.fileContent
            ? _1.generateCustomPageSchemaV4(genericSchema, viewData, serviceAVT, logger)
            : {};
    }
    return schema;
}
