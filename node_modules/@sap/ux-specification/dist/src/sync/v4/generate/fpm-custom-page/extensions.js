...
function isCustomExtensionSchema(schema, appSchema) {
    if (typeof schema.additionalProperties === 'object' && schema.additionalProperties.$ref) {
        const definition = appSchema.definitions?.[utils_1.getDefinitionKey(schema.additionalProperties.$ref)];
...
function getExtensionData(node, schema, property) {
    ...
    const key = utils_3.getAttribute(xmlExtension, PLACEMENT_PROPERTIES.key) || property;
    const anchor = utils_3.getAttribute(xmlExtension, PLACEMENT_PROPERTIES.anchor);
    const placement = utils_3.getAttribute(xmlExtension, PLACEMENT_PROPERTIES.placement) || ux_specification_types_1.v4.Placement.After;
...
function updatePropertyIndices(node, schema) {
    ...
    utils_4.addExtensionToArray(extensions, sortedExtensionsArray, '', 'position');
...
function applyEnumsForAnchors(node, schema, appSchema) {
    ...
    for (const name in schema.properties) {
        ...
        utils_2.addDescriptiveEnumDefinition(appSchema, schema, {
        ...
        for (const entry of oneOf) {
            ...
            if (entry.const.startsWith(annotationPrefix)) {
                ...
                entry.const = utils_3.getAttribute(node.subElements[index], 'key') || entry.const;
            }
        }
    }
}
