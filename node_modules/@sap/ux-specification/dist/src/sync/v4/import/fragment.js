...
function readHeaderFacetExtensions(manifest, target, appId, fragmentFiles) {
    ...
    for (const pageKey in targets) {
        if (pageKey === target) {
            ...
            const configBasedFacets = targetSettings?.controlConfiguration?.[`@${"com.sap.vocabularies.UI.v1.HeaderFacets" /* HeaderFacets */}`]?.facets;
...
function readSectionExtensions(manifest, target, appId, fragmentFiles) {
    ...
    for (const pageKey in targets) {
        if (pageKey === target) {
            ...
            const configBasedSections = targetSettings?.controlConfiguration?.[`@${"com.sap.vocabularies.UI.v1.Facets" /* Facets */}`]?.sections;
