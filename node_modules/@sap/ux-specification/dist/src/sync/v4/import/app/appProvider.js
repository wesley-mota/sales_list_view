...
function convertSettingsToConfig(params) {
    ...
    const schemaId = common_1.getSchemaFilePath(params.page.pageType, params.id, viewId).filename;
    ...
    if (jsonSchema) {
        if (params.page.pageType === ux_specification_types_1.PageTypeV4.ObjectPage) {
            ...
            params.page.config = fpmCustomPage_1.createCustomPageConfig(importParameters);
        }
            ...
            const templateType = common_1.determineV4PageTemplateType(params.target);
            ...
            params.page.config = listReport_1.createListReportConfig(importParameters, schemaType);
        }
    }
}
/**
...
function setEntitySetEntityTypeContextPath(target, page, importParameters) {
    if (target.options) {
        ...
        const { entitySet, entityType } = importProject_1.getEntityTypeEntitySetFromContextPath(importParameters.serviceAVT, page.contextPath, page.entitySet, importParameters.logger);
...
class V4AppProvider extends appProvider_1.AppProvider {
    constructor(importParameters) {
        ...
        utils_1.transferSettingsOfObject(appSettings, importParameters.manifest, appSchema.definitions.AppSettings, '', []);
