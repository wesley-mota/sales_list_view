...
function exportListReportPage(exportParameters, ui5Version) {
    ...
    const listReport = factory.createPageInstance(ux_specification_types_1.PageTypeV2.ListReport, listReportConfig);
    ...
    const pageKeys = [];
    const v2Page = import_1.findListReportPageV2(exportParameters.manifest[ux_specification_types_1.ManifestSection.generic].pages, pageKeys);
    if (!v2Page) {
...
function exportAnalyticalListPage(exportParameters, ui5Version) {
    ...
    const factory = new factory_1.MetadataInstanceFactoryV2();
    //Instantiate Page
    const analyticalListPage = factory.createPageInstance(ux_specification_types_1.PageTypeV2.AnalyticalListPage, analyticalListPageConfig);
    //Instantiate base ID
    const pageLayoutInformation = common_1.getPageLayoutInformation(analyticalListPage);
    const baseId = `${exportParameters.appId}::${pageLayoutInformation.id}::${entitySet}--`;
    const pageKeys = [];
    const v2Page = import_1.findAnalyticalListPage(exportParameters.manifest[ux_specification_types_1.ManifestSection.generic].pages, pageKeys, exportParameters.logger);
    if (!v2Page) {
        return;
    }
    if (!v2Page.component.settings) {
...
function exportObjectPage(exportParameters, ui5Version) {
    ...
    const exportResults = getDefaultExportResult(exportParameters.fragments);
    const factory = new factory_1.MetadataInstanceFactoryV2();
    const objectPage = factory.createPageInstance(ux_specification_types_1.PageTypeV2.ObjectPage, objectPageConfig);
    exportResults.manifest = JSON.parse(JSON.stringify(exportParameters.manifest));
    if (objectPageConfig) {
        const pageLayoutInformation = common_1.getPageLayoutInformation(objectPage);
        const baseId = `${exportParameters.appId}::${pageLayoutInformation.id}::${entitySet}--`;
        const pageKeys = [];
        const v2Page = import_1.findObjectPageV2(exportParameters.manifest[ux_specification_types_1.ManifestSection.generic].pages, entitySet, pageKeys);
        if (!v2Page) {
            return exportResults;
        }
    ...
    if (!validate(overviewPage)) {
        for (const err of validate.errors) {
            ...
            if (err.keyword === 'additionalProperties' &&
                !Object.values(ux_specification_types_1.v2.CardSettingsType).includes(err.params['additionalProperty']) &&
