...
function addHeaderActionCallBackV2(entityType, appSchema, actionAnnotation, headerActions, oDataServiceAVT) {
    return (item, index) => {
        if (!item['Determining'] && item.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAction" /* DataFieldForAction */) {
            ...
            const actionDefinitionKey = `ObjectPageHeaderAction<${common_2.prepareRef(actionId)}>`;
...
function ensureCustomColumnDefinitionExists(appSchema, manifest, entitySetName, entityTypeName, oDataServiceAVT, facet, facetKey, fragments) {
    ...
    const v2Page = import_1.findObjectPageV2(manifest[ux_specification_types_1.ManifestSection.generic].pages, entitySetName, pageKeys);
...
function addSections(generateParameters, appSchema, pages) {
    ...
    if (generateParameters.entitySet.entityType) {
        sections[ux_specification_types_1.SchemaTag.annotationPath] = `/${generateParameters.entitySet.entityType.fullyQualifiedName}/@${"com.sap.vocabularies.UI.v1.Facets" /* Facets */}`;
    }
    if (!pages) {
        return;
    }
    // Get facet annotations
    const facets = common_2.getObjectPageFacets(generateParameters.entitySet.entityType, generateParameters.serviceAVT, ux_specification_types_1.FioriElementsVersion.v2, generateParameters.logger);
