...
function prepareFlexChanges(changes, entitySet, logger) {
    ...
    changes.forEach((changeString) => {
        try {
            ...
            if (change &&
                change.controlId?.includes(ux_specification_types_1.v2.FE_TEMPLATE_V2_OBJECT_PAGE) &&
                ...
                selectorId = common_2.convertSelector(selectorId)
                    .replace(/:3a:3a/g, '::')
...
function matchSelector(schemaDefinition, flexChange, controlIdChecker, selectionIdChecker, sectionId, controlId) {
    ...
    return ((flexChange.controlType === ux_specification_types_1.ControlType.GroupElement && controlIdMatch) ||
        (flexChange.facetId?.includes(ux_specification_types_1.ActionTitlePrefix) && !sectionId && idMatch) ||
...
function fillActions(importParameters, actionProperties, configPart, configParameters, importTypeId, facetId) {
    ...
    if (configParameters.flexChanges && Object.keys(configParameters.flexChanges).length > 0) {
        ...
        for (const actionProperty in actionProperties) {
            const configActionId = common_2.convertActionKey(actionProperty);
...
function fillCustomColumnsInConfig(importParameters, escapedSectionId, configPart, v2Page) {
    ...
    if (customColumnTarget) {
        ...
        common_2.addFragments(configPart[escapedSectionId], importParameters.manifest, v2Page, ux_specification_types_1.PageTypeV2.ObjectPage, importParameters.fragments, objectPageSectionData);
    }
}
/**
