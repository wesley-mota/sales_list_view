...
exports.addFragments = exports.determineTableType = exports.transferSettingsOfObject = exports.addFlex = exports.convertSelector = exports.convertActionKey = void 0;
...
function filterActions(definitionType, jsonSchema) {
    ...
    actions.forEach((definitionActionId) => {
        const fullActionId = common_1.convertActionKeyToStableId(definitionActionId.split('<')[1]?.split('>')[0]);
...
function addSmartChartFlex(config, changePropertyKey, change, jsonSchema) {
    ...
    if (schemaProperty) {
        const syncRule = decorators_1.getReflectMetadata(config[ux_specification_types_1.PropertyName.chart], changePropertyKey);
    ...
    if (appSchema['properties']) {
        for (const propertyKey in appSchema['properties']) {
            ...
            if (syncRule?.manifest) {
                ...
                manifestSection = manifest_1.ensureManifestSectionByPathV2(manifest, path);
...
function determineTableType(manifest, pageKey) {
    ...
    if (manifest[ux_specification_types_1.ManifestSection.generic].pages[pageKey].component.settings &&
        ((manifest[ux_specification_types_1.ManifestSection.generic].pages[pageKey].component.settings.tableSettings &&
