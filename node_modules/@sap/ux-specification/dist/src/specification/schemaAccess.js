...
const getPathToGenericSchema = (version, schemaType, directory = __dirname) => {
    ...
    if (directory.endsWith('dist')) {
        return path_1.join(directory, 'schemas', version, fileName);
    }
    else if (directory.endsWith(path_1.join('specification', 'packages', 'specification'))) {
        return path_1.join(directory, 'dist', 'schemas', version, fileName);
    }
    else if (directory.endsWith(path_1.join('src', 'specification'))) {
        return path_1.join(directory, '..', '..', 'dist', 'schemas', version, fileName);
    }
    else if (directory.indexOf('node_modules') > -1) {
        const moduleRoot = directory.substring(0, directory.indexOf('node_modules') + 12);
        return path_1.join(moduleRoot, '@sap', 'ux-specification', 'dist', 'schemas', version, fileName);
    }
    return undefined;
};
...
const getGenericSchema = async (version, schemaType) => {
    try {
        const path = exports.getPathToGenericSchema(version, schemaType, __dirname);
